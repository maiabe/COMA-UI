...
/*handleReturnTable.set('Handle Query Return', handleQueryReturn);*/ handleReturnTable.set("Database Query Return", handleDatabaseQueryReturn);
...
function handleDatabaseQueryReturn(data, response) {
    ...
    // sort data if needed
    if (data.sortBy) switch(data.sortBy){
        case "iso_date_mid":
...
const noSavedModulesFound = (data)=>postMessage({
        type: "Saved Modules Return",
        clientId: id,
        data: data.response
    });
...
async function getRequest(url1) {
    try {
 ...
 * */ async function getRemoteDropdownOptions(msg) {
    ...
    try {
        ...
        await getCOMAData(url_searchfield, msg.data.data.delay)/*.then(taskResult => {
 ...
 * */ async function queryDatabase(e) {
    ...
    try {
        //var response = undefined;
        await getCOMAData(url_searchfield, e.data.delay)/*.then(taskResult => {
                url_taskResult = url_taskResult + taskResult.task.id;
                const result = getCOMAData(url_taskResult);
                return result;
            })*/ .then((response)=>{
...
async function getCOMATaskData(url1) {
    const response = await fetch(url1, {
        method: "GET",
        headers: {
...
async function postCOMATaskData(url1, body) {
    console.log(body);
    console.log(url1);
    // Default options are marked with *
    const response = await fetch(url1, {
    ...
    const result = await getCOMAData(taskResultUrl);*/ //console.log(result);
...
async function getCOMAData(url1, delay) {
    console.log(url1);
    try {
        ...
        while(responseData.status === "error" && counter < 10){
            /*getCOMAData(url);
            counter++;
            console.log('counter: ' + counter);*/ await new Promise((resolve)=>setTimeout(resolve, delay));
            responseData = await fetch(url1).then((response)=>response.json());
            counter++;
...
async function getPlanetOrbits() {
    const url1 = coma_api + "planets";
    await this.getCOMAData(url1, 5).then((response)=>{
        console.log(response);
...
async function getObjectName(objectID) {
    const url1 = coma_api + "objects/" + objectID;
    console.log(url1);
    return url1;
}

//# sourceMappingURL=clientWorker.c08575ca.js.map
