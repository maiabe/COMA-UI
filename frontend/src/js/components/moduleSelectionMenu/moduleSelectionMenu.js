...
export class ModuleSelectionMenu {
  ...
  initializeMenu = () => {
    this.moduleTypes.forEach((m, index) =>
      this.instantiateMenuButton(m, index)
    );
  };
  ...
  instantiateMenuButton = (m, index) => {
    ...
    if (index === 0)
      button.getButtonElement().classList.add("topRoundedCorners");
    else if (index === this.moduleTypes.length - 1)
      button.getButtonElement().classList.add("bottomRoundedCorners");
...
class ModuleTopButton {
  ...
  createButton = (subMenuItems) => {
    this.wrapperElement = GM.HF.createNewDiv(
      "module-selection-menu-wrapper",
      "module-selection-menu-wrapper",
      ["menuSegmentWrapper"],
      [],
      [],
    ...
    this.buttonElement = GM.HF.createNewDiv(
      `${this.text}_topMenuButton`,
      "",
      ["topMenuButton"],
      [{ style: "backgroundColor", value: this.color }],
      [],
      ""
    );
    this.buttonElement.append(
      GM.HF.createNewIMG("", "", this.image, [], [], "Submenu Icon")
    );
    this.buttonElement.append(
      GM.HF.createNewParagraph("", "", [], [], this.text)
    );
...
class ModuleSubMenu {
  ...
  toggleMenu = () => {
    ...
    this.open = !this.open;
    HTMLFactory.setCustomStyles(this.wrapperElement, [
...
class SubMenuCard {
  ...
  createCard = () => {
    ...
    this.textArea = GM.HF.createNewParagraph(
      "",
      "",
      ["iconText"],
      [],
