...
export class FormCard {
  ...
  #createForm(moduleKey, formName, fields) {
    ...
    fields.forEach((field) => {
      const fieldWrapper = GM.HF.createNewDiv(
        "",
        "",
        ["field-wrapper"],
        [],
      ...
      const fieldLabel = GM.HF.createNewLabel(
        "",
        "",
        [`${fieldInputId}`],
        [],
  ...
  #createFormField(field, fieldInputId) {
    ...
    switch (field.type) {
      case "text":
        ...
        var textInput = GM.HF.createNewTextInput(
          "",
          field.fieldName,
          ["field-input"],
          [{ style: "border", value: "inset" }],
          "text",
          ""
        );
      ...
      case "date":
        ...
        if (field.fieldName === "begin" || field.fieldName === "end") {
          ...
          textInput.setAttribute(
            "id",
            `date-range-${field.fieldName}-${field.index}`
          );
        }
        formField.appendChild(textInput);
        break;
      case "dropdown":
        ...
        var dropdown = GM.HF.createNewSelect(
          "",
          field.fieldName,
          ["field-input"],
          [],
      ...
      case "checkbox":
        formField = GM.HF.createNewDiv(
          fieldInputId,
          "",
          ["checkbox-wrapper", "field-input-wrapper"],
          [],
          [],
          ""
        );
        ...
        checkbox_options.forEach((option) => {
          var checkbox = GM.HF.createNewCheckbox(
            "checkbox-" + field.fieldName + "-" + option.value,
      ...
      case "radio":
        formField = GM.HF.createNewDiv(
          fieldInputId,
          "",
          ["radio-wrapper", "field-input-wrapper"],
          [],
          [],
          ""
        );
        ...
        radio_options.forEach((option) => {
          ...
          var radioButton = GM.HF.createNewRadioButton(
            id,
            field.fieldName,
      ...
      case "typeahead":
        ...
        const resultContainer = GM.HF.createNewDiv(
          "",
          "",
  ...
  #createSubmitButton(formName) {
    const submitButtonWrapper = GM.HF.createNewDiv(
      "",
      "",
      ["button-wrapper"],
      [],
    ...
    const submitButton = GM.HF.createNewButton(
      formName.name + "-btn",
      "",
      ["btn", "form-submit-btn"],
      [],
  ...
  appendFormField(field) {
    ...
    var fieldWrapper = GM.HF.createNewDiv(
      "",
      "",
      ["field-wrapper"],
      [],
    ...
    var label = GM.HF.createNewLabel(
      "",
      "",
      [field.labelName],
      [],
  ...
  updateFormFields(fields) {
    ...
    for (var i = 0; i < fields.length; i++) {
      ...
      const label = GM.HF.createNewLabel(
        "",
        "",
        [`${fieldInputId}`],
        [],
  ...
  appendRemoveField() {
    ...
    fieldWrappers.forEach((field) => {
      // Create remove btn
      var removeFieldIcon = GM.HF.createNewSpan(
        "",
        "",
        ["remove-field-btn"],
        [],
  ...
  appendToolTip(fieldinfo, tooltipElement) {
    ...
    var tooltipSpan = GM.HF.createNewSpan(
      "",
      "",
      ["tooltip-text"],
      [],
    ...
    if (!tooltipElement) {
      tooltipElement = GM.HF.createNewIMG(
        "",
        "",
  ...
  appendMessage(wrapper, message) {
    var formMessage = GM.HF.createNewParagraph(
      "",
      "",
      ["form-message"],
      [],
  ...
  findBeginEndPairs(dateFields) {
    ...
    dateFields.forEach((beginField) => {
      if (beginField.fieldName === "begin") {
        var endField = dateFields.find(
          (field) =>
            field.index === beginField.index && field.fieldName === "end"
        );
        if (endField) {
