...
import {
  ...
  varTest,
  printErrorMessage,
} from "../errorHandling/errorHandlers.js";
import {
  ENVIRONMENT,
  DATA_MANAGER,
  WORKER_MANAGER,
  OUTPUT_MANAGER,
} from "../sharedVariables/constants.js";
...
export class WorkerManager {
  ...
  startWorker = () => {
    if (typeof Worker !== "undefined")
      return new Worker(new URL("./clientWorker.js", import.meta.url));
  ...
  addWorkerToDataTable = (worker) => {
    if (
      invalidVariables(
        [varTest(worker, "worker", "object")],
        "WorkerManager",
        "addWorkerToDataTable"
      )
    )
      return -1;
  ...
  notifyWorkerOfId = (id) => {
    ...
    if (this.#workers.has(id))
      this.#workers
        .get(id)
        .worker.postMessage({ type: "Set Worker Id", id: id });
    return this;
  };
  ...
  setStopWorkerFunction = (id) => {
    if (
      invalidVariables(
        [varTest(id, "id", "number")],
        "WorkerManager",
        "setStopWorkerFunction"
      )
    )
      return undefined;
    if (this.#workers.has(id))
      this.#workers.get(id).stopWorkerFunction = this.stopWorker;
    return this;
  };
  ...
  setHandleReturnFunction = (id, fn) => {
    if (
      invalidVariables(
        [varTest(id, "id", "number")],
        "WorkerManager",
        "setHandleReturnFunction"
      )
    )
      return undefined;
    if (this.#workers.has(id))
      this.#workers.get(id).handleReturnFunction =
        fn === undefined ? this.handleReturn : fn;
    return this;
  };
  ...
  setWorkerReturnMessageRecipient = (id, recipient) => {
    if (
      invalidVariables(
        [
          varTest(id, "id", "number"),
          varTest(recipient, "recipient", "number"),
        ],
        "WorkerManager",
        "setReturnMessageRecipient"
      )
    )
      return undefined;
    if (this.#workers.has(id))
      this.#workers.get(id).returnMessageRecipient = recipient;
    return this;
  };
  ...
  setWorkerReturnMessage = (id, message, moduleKey) => {
    if (
      invalidVariables(
        [
          varTest(id, "id", "number"),
          varTest(message, "message", "string"),
          varTest(moduleKey, "moduleKey", "number"),
        ],
        "WorkerManager",
        "setReturnMessage"
      )
      ...
      workerObject.worker.onmessage = (event) => {
        switch (event.data.type) {
          ...
          case "Processing Complete":
            workerObject.handleReturnFunction(
              event.data.data.results,
              "complete"
            );
            workerObject.handleReturnFunction(
              event.data.data.keys,
              "incomplete"
            );
          ...
          case "Server Return Event":
            this.#sendMessage(
              new Message(
                workerObject.returnMessageRecipient,
                WORKER_MANAGER,
                workerObject.returnMessage.message,
                event.data
              )
            );
            workerObject.stopWorkerFunction(id);
          ...
          case "Remote Dropdown Options Return":
            ...
            localStorage.setItem(
              `Remote Dropdown ${event.data.fieldName}`,
          ...
          case "Planet Orbits Return":
            ...
            if (response.status === "success") {
              ...
              localStorage.setItem(
                "Planet Orbits",
                JSON.stringify(planet_coordinates)
              );
            }
            workerObject.stopWorkerFunction(id);
            break;
  ...
  sendPipelineToServer = (id, pipelineArray) => {
    if (
      invalidVariables(
        [
          varTest(id, "id", "number"),
          varTest(pipelineArray, "pipelineArray", "object"),
        ],
        "WorkerManager",
        "sendPipelineToServer"
      )
    )
      return false;
    if (this.#workers.has(id))
      this.#workers
        .get(id)
        .worker.postMessage({ type: "Execute Pipeline", list: pipelineArray });
  ...
  requestMetadata(id, moduleName) {
    if (this.#workers.has(id))
      this.#workers
        .get(id)
        .worker.postMessage({ type: "Get Metadata", moduleName: moduleName });
  }
  ...
  sendCompositeModuleInfoToServer = (id, groupInfo) => {
    if (this.#workers.has(id))
      this.#workers
        .get(id)
        .worker.postMessage({ type: "Save Module", groupInfo: groupInfo });
  };
  ...
  stopWorker = (id) => {
    ...
    if (id >= 0) {
      if (this.#workers.has(id)) this.#workers.get(id).worker.terminate();
        ...
        console.log(
          `ERROR: cannot delete worker, no worker found for key: ${key}. -- WorkerManager -> stop Worker`
        );
  ...
  requestNewJob = (id, method, url) => {
    ...
    if (this.#workers.has(id)) {
      this.#workers
        .get(id)
        .worker.postMessage({ type: `test`, method: method });
      return true;
    } else return false;
  };
  ...
  handleReturn = (results) => {
    if (
      invalidVariables(
        [
          varTest(results, "results", "object"),
          varTest(event, "event", "string"),
        ],
        "WorkerManager",
        "handleReturn"
      )
