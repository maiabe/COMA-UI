...
class JobManager {
    ...
    setLocalPostHandler() {
        this.handleIncomingPostFunctions.set('Get Objects', this.getObjects.bind(this));
        this.handleIncomingPostFunctions.set('Get Routes', this.getRoutes.bind(this));
        this.handleIncomingPostFunctions.set('Save Module', this.saveModule.bind(this));
        this.handleIncomingPostFunctions.set('Get Saved Modules', this.getSavedModules.bind(this));
        this.handleIncomingPostFunctions.set('Get Metadata', this.getMetadata.bind(this));
    }
    ...
    saveModule = requestBody => this.SM.addModule(requestBody.groupInfo.groupInfo, requestBody.groupInfo.name, requestBody.groupInfo.description);
    getSavedModules = requestBody => this.SM.getAllModules();
    initializeRoutesTable = () => this.addJob({ message: 'routes' });
    populateRoutesTable = routes => Object.entries(routes).forEach(([key, value]) => this.routesTable.set(key, value));
    ...
    initializeWorkerPool(numberOfWorkers, __filename) {
        for (let i = 0; i < numberOfWorkers; i++) {
            ...
            this.workerMap.set(i, { worker: worker, onJob: false, id: worker.threadId, callback: undefined }); // Populate the worker Map
    ...
    addJob(requestBody) {
        if (this.jobTable.has(requestBody.message)) {
            if (this.jobTable.get(requestBody.message).local === true) {
                return this.handleIncomingPostFunctions.get(requestBody.message)(requestBody);
            } else return this.enqueueNewJob(requestBody);
        }
    }

    initializeJobTable() {
        this.jobTable.set('objects', { url: this.getObjectsUrl(), local: false, method: 'GET' });
        this.jobTable.set('routes', { url: 'routes/', callback: this.populateRoutesTable.bind(this), local: false, method: 'GET' });
    ...
    enqueueNewJob(requestBody) {
        if (this.jobTable.has(requestBody.message)) {
            ...
            this.jobQueue.push({ id: id, job: this.jobTable.get(requestBody.message), requestBody: requestBody });
            this.assignNextJob();
            return id;
        }
        return -1;
