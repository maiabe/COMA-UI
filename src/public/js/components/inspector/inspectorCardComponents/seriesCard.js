...
export class SeriesCard {
     ...
     * */
    constructor(moduleKey, fieldName, series) {
        //this.#addSeriesAxisOptions = addSeriesAxisOptions.bind(this);
        this.#createElements(moduleKey, fieldName, series);
        this.#buildCard();
    }

    #createElements(moduleKey, fieldName, series) {
        this.#createWrapper(moduleKey);
        this.#createHeader();
        this.#createContent(moduleKey, fieldName, series);
    }
    ...
    #createWrapper(moduleKey) {
        this.#wrapper = GM.HF.createNewDiv(`series-${moduleKey}`, '', ['series-inspector-wrapper', 'tab-content'], [], [], '');
    }
    ...
    //-- Create Content
    #createContent(moduleKey, fieldName, series) {
        console.log(fieldName);
        console.log(series);
        try {
            let contentWrapper = GM.HF.createNewDiv(fieldName, '', ['series-content-wrapper'], [], [], '');
            let seriesWrapper = GM.HF.createNewDiv('', '', ['series-dropdown-wrapper'], [], [], '');
            ...
            this.#seriesArea = GM.HF.createNewDiv('', '', ['series-area'], [], [], '');

            //-- Append initial content to this series inspector content
            contentWrapper.appendChild(seriesWrapper);
     ...
     *                          (e.g. [{ name: "c-2019-u5", displayName: "C/2019 U5 (PanSTARRS)", dataType: 'category' },..])
     * */
    #createAddSeriesCardFunction(moduleKey, fieldName, series) {
        const button = this.#addSeriesButton;
        button.addEventListener('click', e => {
            ...
            if (selected.value !== 'none') {
                ...
                const xAxisElements = xTraceArea.querySelectorAll('.trace-card-wrapper');
                const xAxisRefs = [];
                xAxisElements.forEach((xEl, i) => {
                    const axisName = xEl.getAttribute('id');
                    const dataType = xEl.querySelector('.field-data-type').value;
                ...
                const yAxisElements = yTraceArea.querySelectorAll('.trace-card-wrapper');
                const yAxisRefs = [];
                yAxisElements.forEach((yEl, i) => {
                    const axisName = yEl.getAttribute('id');
                    const dataType = yEl.querySelector('.field-data-type').value;
    ...
    #createSeriesCard(fieldName, selectedSeries, xAxisRefs, yAxisRefs) {
        ...
        let headerText = GM.HF.createNewSpan('', '', ['series-name'], [], seriesName);
    ...
    #removeSeriesCardFunction(button) {
        button.addEventListener('click', e => {
            ...
            let seriesArea = e.target.closest('.series-area');
...
/*
    ...
    getCard() {
        return { card: this, wrapper: this.#wrapper, content: this.#seriesContent, seriesDropdown: this.#seriesDropdown, seriesArea: this.#seriesArea };
    }
}
