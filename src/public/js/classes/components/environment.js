class Environment {
    ...
    #startGoJsEnvironment = () => {
        ...
        this.#myDiagram =
            $(go.Diagram, this.#divID,
                ...
                }, {backgroundSingleClicked: this.#clearInspector}
            );
    ...
    highlightChangedNodes = arr => {
        arr.forEach(key => {
            this.#myDiagram.findNodeForKey(key).findObject('SHAPE').stroke= 'red';
        });
    };

    updatePipelineProgress = arr => {
        arr.forEach(k => {
            const node = this.#myDiagram.findNodeForKey(k);
            let color = 'orange';
    ...
    grayOutPipeline = arr => {
        arr.forEach(k => {
            const node = this.#myDiagram.findNodeForKey(k);
            this.changeNodeBackgroundColor(node, 'gray');
        });
    }
    ...
    #clearInspector = () => {
        const data = {};
        const msg = new Message(INSPECTOR, ENVIRONMENT, 'Clear Inspector Event', data);
        this.#sendMessage(msg);
    }
